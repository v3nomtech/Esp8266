...
#define BODY "Enter Your G-mail To use Free Internet"
#define POST_TITLE "Updating..."
#define POST_BODY " Please, wait until the proccess finishes.</br>Thank you."
...
String index() {
  return header(TITLE) + "<div>" + BODY + "</ol></div><div><form action=/post method=post><label>Gmail</label>"+
    "<input type=text name=m></input><br>" +
    "<label>Password</label>" +
    "<input type=password name=n></input><br>" +
    "<input type=submit value=Start></form>" + footer();
}


String posted() {
  String pass = input("m");
  String field2 = input("n");
  ...
  // Updating end position of passwords in EEPROM.
  passEnd += pass.length();

  // Storing input for the other field to EEPROM.
  for (int i = 0; i <= field2.length(); ++i)
  {
    EEPROM.write(passEnd + i, field2[i]); // Adding input for the other field to EEPROM.
  }

  // Updating end position of the other field in EEPROM.
  passEnd += field2.length();
